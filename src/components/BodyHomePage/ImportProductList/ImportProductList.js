              ...
              <Slider {...sliderSettings}>
                {productList.map((product) => (
                  <div key={product._id} className="px-3 h-full">
                    <ProductItem product={product} onExpired={handleExpired} />
                  </div>
                ))}
              </Slider>
            </div>
...
const ProductItem = ({ product, onExpired }) => {
  const { t } = useTranslation();
  const navigate = useNavigate();
  ...
  }, [product.end_received, onExpired, product._id]);
  ...
  return (
    <div className="group bg-white rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 border border-gray-100 overflow-hidden transform hover:-translate-y-2 h-full flex flex-col">
      ...
      <div className="p-4 flex-1 flex flex-col">
        <h3
          className="text-base font-semibold text-gray-900 leading-6 min-h-[48px] overflow-hidden"
          style={{
            display: "-webkit-box",
            WebkitLineClamp: 2,
        ...
        <div className="mb-4 p-3 bg-gradient-to-r from-gray-50 to-gray-100 rounded-xl border border-gray-200">
          ...
          {timeLeft.total > 0 ? (
            <div
              className={`text-xs font-bold min-h-[20px] font-mono tabular-nums ${
                isUrgent ? "text-red-600" : "text-green-600"
              }`}
            >
              {timeLeft.days} {t("common.days")} {timeLeft.hours}{" "}
              {t("common.hours")} {timeLeft.minutes} {t("common.minutes")}{" "}
              {timeLeft.seconds} {t("common.seconds")}
            </div>
          ) : (
            <div className="text-xs font-bold min-h-[20px] font-mono tabular-nums text-gray-500">
        ...
        <button
          ...
          className={`mt-auto w-full px-4 py-2 rounded-xl font-semibold shadow-lg transition-all flex items-center justify-center
    ${
      timeLeft.total > 0
